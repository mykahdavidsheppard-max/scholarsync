// ============================================================
// SCHOLARSYNC — app.js (Firebase Compat SDK — NO modules needed)
// ============================================================
...
async function signUp(){
  ...
  try{
    ...
    const schoolSnap=await db.collection('users').where('school','==',school).get();
...
function renderDeadlines(){
  ...
  document.getElementById('dl-list').innerHTML=items.length?items.map(dl=>{const days=Math.ceil((new Date(dl.date)-new Date())/86400000);const cls=days<=3?'dl-red':days<=7?'dl-gold':'dl-blue';const label=days<0?'PAST DUE':days===0?'TODAY':days===1?'TOMORROW':`${days}`;return`<div class="deadline-card"><div class="deadline-days ${cls}">${label}</div><div class="deadline-info"><div class="deadline-name">${dl.name}</div><div class="deadline-meta">${dl.cls} • ${dl.type} • ${dl.date}</div></div><button class="btn-ghost btn-sm" onclick="deleteDeadline('${dl.id}')">×</button></div>`;}).join(''):'<div class="empty-state">No deadlines yet</div>';
  renderDashDeadlines();
}
...
function renderDashDeadlines(){const items=lsArr('ss_deadlines_'+currentUser.uid).sort((a,b)=>new Date(a.date)-new Date(b.date)).slice(0,5);const el=document.getElementById('dash-deadlines');if(!el)return;el.innerHTML=items.length?items.map(dl=>{const days=Math.ceil((new Date(dl.date)-new Date())/86400000);const cls=days<=3?'dl-red':days<=7?'dl-gold':'dl-blue';return`<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px;"><span>${dl.name}</span><span class="${cls}" style="font-weight:700">${days<=0?'TODAY':days+'d'}</span></div>`;}).join(''):'<div class="empty-state">No upcoming deadlines</div>';document.getElementById('stat-due').textContent=items.filter(d=>Math.ceil((new Date(d.date)-new Date())/86400000)<=7&&Math.ceil((new Date(d.date)-new Date())/86400000)>=0).length;}

// ---- EXAMS ----
function renderExams(){const items=lsArr('ss_exams_'+currentUser.uid).sort((a,b)=>new Date(a.date)-new Date(b.date));document.getElementById('exam-list').innerHTML=items.length?items.map(ex=>{const days=Math.ceil((new Date(ex.date)-new Date())/86400000);const cls=days<=3?'#ef4444':days<=7?'#f59e0b':'#3b82f6';return`<div class="exam-card"><div class="exam-days" style="color:${cls}">${days===0?'TODAY!':days<0?'PAST':days}</div><div style="font-size:11px;color:var(--gray);">${days>0?'DAYS LEFT':''}</div><div class="exam-name">${ex.name}</div><div class="exam-subject">${ex.subject}</div><div class="exam-date">${ex.date}</div><button class="btn-ghost btn-sm" style="margin-top:10px;width:100%;" onclick="deleteExam('${ex.id}')">Delete</button></div>`;}).join(''):'<div class="empty-state">No exams yet</div>';}
...
function renderCalendar(){
  ...
  const first=new Date(calYear,calMonth,1).getDay();const daysInMonth=new Date(calYear,calMonth+1,0).getDate();const today=new Date();
  ...
  for(let d=1;d<=daysInMonth;d++){
    ...
    div.onclick=()=>openCalDay(dateStr,months[calMonth]+' '+d);grid.appendChild(div);
  }
}
function changeMonth(dir){calMonth+=dir;if(calMonth>11){calMonth=0;calYear++;}if(calMonth<0){calMonth=11;calYear--;}renderCalendar();}
function openCalDay(date,label){selectedCalDate=date;document.getElementById('cal-modal-title').textContent=`Events for ${label}`;const events=lsArr('ss_calEvents_'+currentUser.uid).filter(e=>e.date===date);const holidays=HOLIDAYS.filter(h=>h.date===date);document.getElementById('cal-modal-events').innerHTML=[...holidays,...events].length?[...holidays,...events].map(e=>`<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);font-size:13px;"><span>${e.title}</span>${e.uid===currentUser.uid?`<button class="btn-ghost btn-sm" onclick="deleteCalEvent('${e.id}')">×</button>`:''}</div>`).join(''):'<div class="empty-state" style="padding:8px 0;">No events</div>';openModal('cal-modal');}
...
function renderAnnouncements(){
  const unsub=db.collection('announcements').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{
...
async function postAnnouncement(){if(!currentUserData.isAdmin)return;const title=document.getElementById('ann-title').value.trim();const body=document.getElementById('ann-body').value.trim();if(!title||!body)return;await db.collection('announcements').add({title,body,postedBy:currentUser.uid,postedByName:currentUserData.name,school:currentUserData.school,createdAt:FS.serverTimestamp()});closeAllModals();document.getElementById('ann-title').value='';document.getElementById('ann-body').value='';}
...
function renderClassrooms(){
  ...
  const q=(currentUserData.isTeacher||currentUserData.isAdmin)?db.collection('classrooms').where('school','==',school).where('teacherUid','==',uid):db.collection('classrooms').where('school','==',school).where('students','array-contains',uid);
...
async function openClassroom(id){const snap=await db.collection('classrooms').doc(id).get();if(!snap.exists)return;currentClass={id,...snap.data()};document.getElementById('cd-title').textContent=currentClass.name;document.getElementById('cd-sub').textContent=currentClass.teacherName+' • '+currentClass.subject;const isTeacher=(currentUserData.isTeacher||currentUserData.isAdmin)&&currentClass.teacherUid===currentUser.uid;document.querySelectorAll('.teacher-only').forEach(el=>el.style.display=isTeacher?'':'none');renderClassroomDetail();nav('classroom-detail');}
...
async function submitPassRequest(){
  ...
  const ref=await db.collection('hallpasses').add({passId,studentUid:currentUser.uid,studentName:currentUserData.name,classId:currentClass.id,destination:selectedDest,duration:selectedDestMins,status:'pending',school:currentUserData.school,requestedAt:FS.serverTimestamp(),message:null,used:false});
  activePassId=ref.id;
  document.getElementById('dest-grid').style.display='none';document.getElementById('pass-request-screen').style.display='none';document.getElementById('pass-pending-screen').style.display='';
  const unsub=db.collection('hallpasses').doc(ref.id).onSnapshot(snap=>{
    ...
    if(data&&data.status==='denied'){unsub();document.getElementById('pass-pending-screen').style.display='none';document.getElementById('pass-request-screen').style.display='';alert('Pass request denied by teacher.');nav('classroom');}
  });
}
function showActivePass(pass){
  ...
  document.getElementById('pass-avatar').textContent=currentUserData.name[0];document.getElementById('pass-avatar').style.background=currentUserData.avatar;
  ...
  document.getElementById('pass-issued').textContent=issued;document.getElementById('pass-limit').textContent=pass.duration+' min';document.getElementById('pass-approved-by').textContent=pass.approvedBy||'Teacher';
  document.getElementById('desktop-dest-name').textContent=pass.destination;document.getElementById('desktop-pass-id').textContent='#'+pass.passId;document.getElementById('desktop-student-name').textContent=currentUserData.name;document.getElementById('desktop-issued').textContent=issued;document.getElementById('desktop-limit').textContent=pass.duration+' min';document.getElementById('desktop-approved').textContent=pass.approvedBy||'Teacher';document.getElementById('pass-id-desktop').textContent='#'+pass.passId;
  passTotal=pass.duration*60;passRemaining=passTotal;clearInterval(passTimerInterval);passTimerInterval=setInterval(tickPass,1000);
}
...
function updatePassTimer(secs){
  ...
  document.getElementById('pass-timer-num').textContent=`${m}:${pad(s)}`;document.getElementById('pass-ring-pct').textContent=Math.round(pct*100)+'%';document.getElementById('desktop-timer-num').textContent=`${m}:${pad(s)}`;
...
async function renderPassPanels(){
  if(!currentClass)return;
  const snap=await db.collection('hallpasses').where('classId','==',currentClass.id).get();
  const passes=snap.docs.map(d=>({id:d.id,...d.data()}));
  const pending=passes.filter(p=>p.status==='pending'),active=passes.filter(p=>p.status==='active');
  const pEl=document.getElementById('pending-passes');if(pEl)pEl.innerHTML=pending.length?pending.map(p=>`<div class="pass-panel"><div class="pass-panel-info"><div class="pass-panel-name">${p.studentName}</div><div class="pass-panel-dest">${p.destination} • ${p.duration} min</div></div><div class="pass-panel-btns"><button class="btn-blue btn-sm" onclick="approvePass('${p.id}')">Approve</button><button class="btn-red btn-sm" onclick="denyPass('${p.id}')">Deny</button></div></div>`).join(''):'<div class="empty-state">No pending requests</div>';
  const aEl=document.getElementById('active-passes');if(aEl)aEl.innerHTML=active.length?active.map(p=>`<div class="pass-panel"><div class="pass-panel-info"><div class="pass-panel-name">${p.studentName}</div><div class="pass-panel-dest">${p.destination}<br><span style="font-size:11px;color:var(--gray)">#${p.passId}</span></div></div><div class="pass-panel-btns"><button class="btn-ghost btn-sm" onclick="sendTeacherMsg('${p.id}')">Msg</button><button class="btn-red btn-sm" onclick="endPassTeacher('${p.id}')">End</button></div></div>`).join(''):'<div class="empty-state">No active passes</div>';
  renderTPPanels();
}
async function approvePass(id){await db.collection('hallpasses').doc(id).update({status:'active',approvedAt:FS.serverTimestamp(),approvedBy:currentUserData.name});renderPassPanels();}
async function denyPass(id){await db.collection('hallpasses').doc(id).update({status:'denied',used:true});renderPassPanels();}
async function endPassTeacher(id){await db.collection('hallpasses').doc(id).update({status:'completed',used:true});renderPassPanels();}
...
async function verifyPassById(passId,resultElId){
  const el=document.getElementById(resultElId);el.innerHTML='<div style="color:var(--gray);font-size:13px;">Checking...</div>';
  const snap=await db.collection('hallpasses').where('passId','==',passId).where('school','==',currentUserData.school).get();
  if(snap.empty){el.innerHTML=`<div class="verify-invalid">${ICONS.x} INVALID — Pass #${passId} does not exist or was never issued.</div>`;return;}
  ...
  else if(pass.status==='completed'||pass.used)el.innerHTML=`<div class="verify-expired">${ICONS.warn} EXPIRED — Pass #${passId} has already been used.</div>`;
  else if(pass.status==='pending')el.innerHTML=`<div class="verify-expired">${ICONS.clock} PENDING — Pass #${passId} awaiting teacher approval.</div>`;
...
async function renderTPPanels(){
  const snap=await db.collection('hallpasses').where('school','==',currentUserData.school).get();
  const passes=snap.docs.map(d=>({id:d.id,...d.data()}));
  const pending=passes.filter(p=>p.status==='pending'),active=passes.filter(p=>p.status==='active');
  const pEl=document.getElementById('tp-pending');if(pEl)pEl.innerHTML=pending.length?pending.map(p=>`<div class="pass-panel"><div class="pass-panel-info"><div class="pass-panel-name">${p.studentName}</div><div class="pass-panel-dest">${p.destination} • ${p.duration} min</div></div><div class="pass-panel-btns"><button class="btn-blue btn-sm" onclick="approvePass('${p.id}');renderTPPanels()">Approve</button><button class="btn-red btn-sm" onclick="denyPass('${p.id}');renderTPPanels()">Deny</button></div></div>`).join(''):'<div class="empty-state">No pending requests</div>';
  const aEl=document.getElementById('tp-active');if(aEl)aEl.innerHTML=active.length?active.map(p=>`<div class="pass-panel"><div class="pass-panel-info"><div class="pass-panel-name">${p.studentName}</div><div class="pass-panel-dest">${p.destination}<br><span style="font-size:11px;color:var(--gray)">#${p.passId}</span></div></div><div class="pass-panel-btns"><button class="btn-ghost btn-sm" onclick="sendTeacherMsg('${p.id}')">Msg</button><button class="btn-red btn-sm" onclick="endPassTeacher('${p.id}');renderTPPanels()">End</button></div></div>`).join(''):'<div class="empty-state">No active passes</div>';
}

// ---- CLUBS ----
function renderClubs(){const unsub=db.collection('clubs').where('school','==',currentUserData.school).onSnapshot(snap=>{const clubs=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('clubs-list').innerHTML=clubs.length?clubs.map(c=>`<div class="club-card"><div class="club-emoji">${c.emoji}</div><div class="club-name">${c.name}</div><div class="club-desc">${c.description}</div><div class="club-role-badge">${c.president===currentUser.uid?'President':(c.members||[]).includes(currentUser.uid)?'Member':'Not joined'}</div></div>`).join(''):'<div class="empty-state">No clubs yet</div>';});unsubscribers.push(unsub);}
async function createClub(){if(!currentUserData.isAdmin)return;const name=document.getElementById('club-name').value.trim();const description=document.getElementById('club-desc').value.trim();const emoji=document.getElementById('club-emoji').value||'🎯';if(!name)return;await db.collection('clubs').add({name,description,emoji,president:currentUser.uid,members:[currentUser.uid],school:currentUserData.school,createdAt:FS.serverTimestamp()});closeAllModals();['club-name','club-desc','club-emoji'].forEach(id=>document.getElementById(id).value='');}

// ---- SPORTS ----
function renderSports(){const unsub=db.collection('sports').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('sports-list').innerHTML=items.length?items.map(s=>{const result=parseInt(s.ourScore)>parseInt(s.theirScore)?'WIN':parseInt(s.ourScore)<parseInt(s.theirScore)?'LOSS':'TIE';const cls=result==='WIN'?'wlt-win':result==='LOSS'?'wlt-loss':'wlt-tie';return`<div class="sport-card"><div class="sport-header"><span class="sport-tag">${s.sport}</span><span class="wlt-badge ${cls}">${result}</span></div><div class="score-row"><div class="score-team"><div class="score-team-name">Panthers</div><div class="score-num">${s.ourScore}</div></div><div class="score-vs">VS</div><div class="score-team"><div class="score-team-name">${s.opponent}</div><div class="score-num">${s.theirScore}</div></div></div><div style="font-size:12px;color:var(--gray);margin-top:10px;">${s.date}</div>${currentUserData.isAdmin?`<button class="btn-ghost btn-sm" style="margin-top:8px;" onclick="deleteSport('${s.id}')">Delete</button>`:''}</div>`;}).join(''):'<div class="empty-state">No game results yet</div>';});unsubscribers.push(unsub);}
async function addSportResult(){const sport=document.getElementById('sport-type').value;const opponent=document.getElementById('sport-opp').value.trim();const ourScore=document.getElementById('sport-us').value;const theirScore=document.getElementById('sport-them').value;const date=document.getElementById('sport-date').value;if(!opponent||!ourScore||!theirScore||!date)return;await db.collection('sports').add({sport,opponent,ourScore,theirScore,date,school:currentUserData.school,postedBy:currentUser.uid,createdAt:FS.serverTimestamp()});closeAllModals();['sport-opp','sport-us','sport-them','sport-date'].forEach(id=>document.getElementById(id).value='');}
async function deleteSport(id){await db.collection('sports').doc(id).delete();}

// ---- POLLS ----
function renderPolls(){const unsub=db.collection('polls').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('polls-list').innerHTML=items.length?items.map(p=>{const totalVotes=Object.values(p.votes||{}).length;const voted=(p.votes||{})[currentUser.uid]!==undefined;const optHTML=p.options.map((opt,i)=>{const count=Object.values(p.votes||{}).filter(v=>v===i).length;const pct=totalVotes?Math.round(count/totalVotes*100):0;return`<div class="poll-opt" onclick="${!voted?`castVote('${p.id}',${i})`:''}"><div class="poll-opt-label"><span>${opt}</span><span>${pct}%</span></div><div class="poll-bar"><div class="poll-fill" style="width:${pct}%"></div></div></div>`;}).join('');return`<div class="poll-card"><div class="poll-q">${p.question}</div>${optHTML}<div style="font-size:12px;color:var(--gray);margin-top:8px;">${totalVotes} votes${currentUserData.isAdmin?` • <button class="btn-ghost btn-sm" onclick="deletePoll('${p.id}')">Delete</button>`:''}</div></div>`;}).join(''):'<div class="empty-state">No polls yet</div>';});unsubscribers.push(unsub);}
async function createPoll(){if(!currentUserData.isAdmin)return;const q=document.getElementById('poll-q').value.trim();const opts=['poll-o1','poll-o2','poll-o3','poll-o4'].map(id=>document.getElementById(id).value.trim()).filter(Boolean);if(!q||opts.length<2)return;await db.collection('polls').add({question:q,options:opts,votes:{},school:currentUserData.school,createdAt:FS.serverTimestamp(),postedBy:currentUser.uid});closeAllModals();['poll-q','poll-o1','poll-o2','poll-o3','poll-o4'].forEach(id=>document.getElementById(id).value='');}
...
// ---- DIRECTORY ----
async function renderDirectory(){const snap=await db.collection('users').where('school','==',currentUserData.school).get();displayDir(snap.docs.map(d=>d.data()).filter(u=>u.uid!==currentUser.uid));}
function displayDir(users){document.getElementById('dir-list').innerHTML=users.length?users.map(u=>`<div class="dir-item"><div class="dir-avatar" style="background:${u.avatar}">${u.name[0]}</div><div style="flex:1"><div class="dir-name">${u.name}</div><div class="dir-school">${u.email}</div></div><span class="role-badge ${u.isAdmin?'role-admin':u.isTeacher?'role-teacher':'role-student'}">${u.isAdmin?'Admin':u.isTeacher?'Teacher':'Student'}</span></div>`).join(''):'<div class="empty-state">No students found</div>';}
...
// ---- LOST & FOUND ----
function renderLostFound(){const unsub=db.collection('lostfound').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('lf-list').innerHTML=items.length?items.map(i=>`<div class="lf-card"><div class="lf-status-badge ${i.status==='Lost'?'lf-lost':'lf-found'}">${i.status.toUpperCase()}</div><div style="flex:1"><div class="lf-name">${i.name}</div><div class="lf-loc">${i.location} • ${i.postedByName}</div></div>${i.postedBy===currentUser.uid||currentUserData.isAdmin?`<button class="btn-ghost btn-sm" onclick="deleteLF('${i.id}')">×</button>`:''}</div>`).join(''):'<div class="empty-state">No items posted</div>';});unsubscribers.push(unsub);}
async function addLF(){const name=document.getElementById('lf-item').value.trim();const location=document.getElementById('lf-loc').value.trim();const status=document.getElementById('lf-status').value;if(!name)return;await db.collection('lostfound').add({name,location,status,postedBy:currentUser.uid,postedByName:currentUserData.name,school:currentUserData.school,createdAt:FS.serverTimestamp()});closeAllModals();['lf-item','lf-loc'].forEach(id=>document.getElementById(id).value='');}
async function deleteLF(id){await db.collection('lostfound').doc(id).delete();}

// ---- CARPOOL ----
function renderCarpool(){const unsub=db.collection('carpool').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('cp-list').innerHTML=items.length?items.map(i=>`<div class="cp-card"><div class="cp-icon">${parseInt(i.seats)>0?ICONS.car:ICONS.hand}</div><div style="flex:1"><div class="cp-area">${i.area}</div><div class="cp-info">${i.contact}</div></div><div class="cp-seats-badge">${parseInt(i.seats)>0?i.seats+' seats':'Looking'}</div>${i.postedBy===currentUser.uid?`<button class="btn-ghost btn-sm" onclick="deleteCP('${i.id}')">×</button>`:''}</div>`).join(''):'<div class="empty-state">No carpool posts yet</div>';});unsubscribers.push(unsub);}
async function addCarpool(){const area=document.getElementById('cp-area').value.trim();const seats=document.getElementById('cp-seats').value;const contact=document.getElementById('cp-contact').value.trim();if(!area||!contact)return;await db.collection('carpool').add({area,seats:seats||'0',contact,postedBy:currentUser.uid,school:currentUserData.school,createdAt:FS.serverTimestamp()});closeAllModals();['cp-area','cp-seats','cp-contact'].forEach(id=>document.getElementById(id).value='');}
async function deleteCP(id){await db.collection('carpool').doc(id).delete();}

// ---- GROUP CHATS ----
function renderChats(){const unsub=db.collection('chats').where('school','==',currentUserData.school).where('memberEmails','array-contains',currentUserData.email).onSnapshot(snap=>{const chats=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('chat-list').innerHTML=chats.length?chats.map(c=>{const daysLeft=Math.max(0,14-Math.floor((Date.now()-(c.createdAt?.toDate?c.createdAt.toDate().getTime():Date.now()))/86400000));return`<div class="chat-item" onclick="openChat('${c.id}')"><div class="chat-item-name">${c.name}</div><div class="chat-item-preview">${c.lastMessage||'No messages yet'}</div><div class="chat-item-meta"><span></span><span class="chat-expires-badge">${daysLeft}d left</span></div></div>`;}).join(''):'<div class="empty-state">No chats yet</div>';});unsubscribers.push(unsub);}
async function createChat(){const name=document.getElementById('new-chat-name').value.trim();if(!name)return;const memberEmails=[currentUserData.email,...chatMembers];await db.collection('chats').add({name,memberEmails,school:currentUserData.school,createdAt:FS.serverTimestamp(),lastMessage:null});chatMembers=[];document.getElementById('chat-members-list').innerHTML='';renderChats();closeAllModals();document.getElementById('new-chat-name').value='';}
async function addChatMember(){const email=document.getElementById('chat-member-email').value.trim();if(!email)return;const snap=await db.collection('users').where('email','==',email).where('school','==',currentUserData.school).get();if(snap.empty)return alert('No user found with that email at your school.');if(chatMembers.includes(email))return alert('Already added!');const user=snap.docs[0].data();chatMembers.push(email);document.getElementById('chat-members-list').innerHTML+=`<div style="display:flex;align-items:center;gap:8px;padding:6px 0;font-size:13px;border-bottom:1px solid var(--border);"><div class="dir-avatar" style="background:${user.avatar};width:28px;height:28px;font-size:11px;">${user.name[0]}</div>${user.name} <span style="color:var(--gray)">(${email})</span></div>`;document.getElementById('chat-member-email').value='';}
async function openChat(id){
  currentChat=id;const chatSnap=await db.collection('chats').doc(id).get();if(!chatSnap.exists)return;const chat=chatSnap.data();
  ...
  const unsub=db.collection('chats').doc(id).collection('messages').orderBy('sentAt').onSnapshot(snap=>{const messages=snap.docs.map(d=>({id:d.id,...d.data()}));const el=document.getElementById('chat-messages');if(!el)return;el.innerHTML=messages.length?messages.map(m=>`<div class="chat-msg ${m.uid===currentUser.uid?'me':'other'}">${m.uid!==currentUser.uid?`<div class="chat-msg-sender">${m.name}</div>`:''}<div class="chat-msg-bubble">${m.text}</div></div>`).join(''):'<div class="empty-state">No messages yet. Say something!</div>';el.scrollTop=el.scrollHeight;});
  unsubscribers.push(unsub);
}
async function sendChatMsg(){const input=document.getElementById('chat-input');if(!input||!input.value.trim()||!currentChat)return;const text=input.value.trim();input.value='';await db.collection('chats').doc(currentChat).collection('messages').add({text,uid:currentUser.uid,name:currentUserData.name,sentAt:FS.serverTimestamp()});await db.collection('chats').doc(currentChat).update({lastMessage:text});}

// ---- STUDY GROUPS ----
function renderStudyGroups(){const unsub=db.collection('studygroups').where('school','==',currentUserData.school).orderBy('createdAt','desc').onSnapshot(snap=>{const items=snap.docs.map(d=>({id:d.id,...d.data()}));document.getElementById('sg-list').innerHTML=items.length?items.map(s=>{const full=(s.members||[]).length>=s.spots;return`<div class="sg-card"><div class="sg-info"><div class="sg-subject">${s.subject}</div><div class="sg-details">📍 ${s.location}</div><div class="sg-details">${(s.members||[]).length}/${s.spots} spots</div></div><span class="sg-spots-badge ${full?'sg-full':'sg-open'}">${full?'FULL':'OPEN'}</span>${!full&&!(s.members||[]).includes(currentUser.uid)?`<button class="btn-blue btn-sm" onclick="joinStudyGroup('${s.id}')">Join</button>`:''} ${s.postedBy===currentUser.uid?`<button class="btn-ghost btn-sm" onclick="deleteSG('${s.id}')">×</button>`:''}</div>`;}).join(''):'<div class="empty-state">No study groups yet</div>';});unsubscribers.push(unsub);}
...
// ---- REMINDERS ----
function renderReminders(){const items=lsArr('ss_reminders_'+currentUser.uid).sort((a,b)=>new Date(a.date)-new Date(b.date));document.getElementById('rem-list').innerHTML=items.length?items.map(r=>{const days=r.date?Math.ceil((new Date(r.date)-new Date())/86400000):null;const cls=days===null?'var(--gray)':days<=0?'var(--red)':days===1?'var(--gold)':'var(--blue-light)';const label=days===null?'—':days<=0?'TODAY':days===1?'TOMORROW':days;return`<div class="rem-card"><div class="rem-days" style="color:${cls}">${label}</div><div style="flex:1"><div class="rem-text">${r.text}</div>${r.date?`<div class="rem-date">${r.date}</div>`:''}</div><button class="btn-ghost btn-sm" onclick="deleteReminder('${r.id}')">×</button></div>`;}).join(''):'<div class="empty-state">No reminders yet</div>';}
