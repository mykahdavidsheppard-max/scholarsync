  ...
  const unsub=q.onSnapshot(snap=>{
    ...
    el.innerHTML=chats.length?chats.map(c=>{
      ...
      return`<div class="chat-item" onclick="openChat('${c.id}')">
        <div class="chat-item-name">${c.name}${isStaff&&c.createdByName?` <span style="font-size:10px;color:var(--gray);">by ${c.createdByName}</span>`:''}${c.deleted?` <span style="font-size:10px;color:var(--red);">[DELETED]</span>`:''}</div>
