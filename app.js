...
function signUp() {
  ...
  // Check if email was pre-granted teacher or admin by an existing admin
  const grantedTeachers = lsArr('ss_approvedTeachers_' + school).map(e => e.toLowerCase());
  const grantedAdmins = lsArr('ss_approvedAdmins_' + school).map(e => e.toLowerCase());
  ...
  const role = (isAdmin || isGrantedAdmin) ? 'admin' : isGrantedTeacher ? 'teacher' : 'student';
  const user = {
    id: genId(), name, email, password: pass, school, grade,
    role: isAdmin || isGrantedAdmin ? 'teacher' : isGrantedTeacher ? 'teacher' : 'student',
    isAdmin: isAdmin || isGrantedAdmin,
    isTeacher: isGrantedTeacher || isAdmin || isGrantedAdmin,
...
function setupUI() {
  ...
  if (u.isTeacher || u.role === 'teacher' || u.isAdmin) {
...
async function loadWeather() {
  ...
  try {
    ...
    const SVG = {
      ...
      snow: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="22" height="22"><line x1="12" y1="2" x2="12" y2="22"/><path d="M17 7l-5 5-5-5M17 17l-5-5-5 5"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M7 7l5 5 5-5M7 17l5-5 5 5"/></svg>`,
    ...
    let cond = `${SVG.sun} Clear`, desc = 'Clear skies';
    if (code >= 1 && code <= 3) { cond = `${SVG.cloud} Partly Cloudy`; desc = 'Partly cloudy'; }
    if (code >= 51 && code <= 67) { cond = `${SVG.rain} Rain`; desc = 'Rainy'; }
    if (code >= 71 && code <= 77) { cond = `${SVG.snow} Snow`; desc = 'Snowing'; }
    if (code >= 80 && code <= 82) { cond = `${SVG.rain} Showers`; desc = 'Light showers'; }
    if (code >= 95) { cond = `${SVG.storm} Thunderstorm`; desc = 'Thunderstorms'; }
    card.innerHTML = `
      <div style="display:flex;align-items:flex-start;gap:20px;">
        <div>
          <div class="weather-big-temp">${tempF}°F</div>
          <div class="weather-condition">${cond}</div>
          <div style="font-size:13px;color:var(--gray)">${SVG.pin} Myrtle Beach, SC</div>
        </div>
      </div>
...
function verifyPassById(passId, resultElId) {
  ...
  if (!pass) { el.innerHTML = `<div class="verify-invalid">${ICONS.x} INVALID — Pass ID #${passId} does not exist or was never issued.</div>`; return; }
  if (pass.status === 'active') { el.innerHTML = `<div class="verify-valid">${ICONS.check} VALID — Pass #${passId} is currently active.<br><strong>Student:</strong> ${pass.studentName}<br><strong>Destination:</strong> ${pass.destination}<br><strong>Issued:</strong> ${new Date(pass.approvedAt).toLocaleTimeString()}<br><strong>Duration:</strong> ${pass.duration} min</div>`; }
...
function approveTeacher() {
  ...
  if (teachers.includes(email)) return alert('Already granted teacher access!');
  teachers.push(email);
  ls('ss_approvedTeachers_' + currentUser.school, teachers);
  // If user already signed up, upgrade them now
  const users = lsArr('ss_users');
  const user = users.find(u => u.email.toLowerCase() === email && u.school === currentUser.school);
  if (user) { user.role = 'teacher'; user.isTeacher = true; user.approved = true; ls('ss_users', users); }
  renderApprovedTeachers();
  document.getElementById('teacher-email-input').value = '';
  alert('Teacher access granted! They can now access teacher features.');
}
function grantAdmin() {
  const email = document.getElementById('admin-email-input').value.trim().toLowerCase();
  if (!email) return;
  const admins = lsArr('ss_approvedAdmins_' + currentUser.school);
  if (admins.includes(email)) return alert('Already an admin!');
  admins.push(email);
  ls('ss_approvedAdmins_' + currentUser.school, admins);
  // Also grant teacher while we're at it
  const teachers = lsArr('ss_approvedTeachers_' + currentUser.school);
  if (!teachers.includes(email)) { teachers.push(email); ls('ss_approvedTeachers_' + currentUser.school, teachers); }
  // If user already signed up, upgrade them now
  const users = lsArr('ss_users');
  const user = users.find(u => u.email.toLowerCase() === email && u.school === currentUser.school);
  if (user) { user.isAdmin = true; user.isTeacher = true; user.role = 'teacher'; user.approved = true; ls('ss_users', users); }
  renderApprovedAdmins();
  document.getElementById('admin-email-input').value = '';
  alert('Admin access granted! They now have full admin powers.');
}
...
function removeAdmin(email) {
  ls('ss_approvedAdmins_' + currentUser.school, lsArr('ss_approvedAdmins_' + currentUser.school).filter(e => e !== email));
  const users = lsArr('ss_users');
  const user = users.find(u => u.email.toLowerCase() === email && u.school === currentUser.school);
  if (user && user.email !== 'mykahsheppard@g.horrycountyschools.net') { user.isAdmin = false; ls('ss_users', users); }
  renderApprovedAdmins();
}
function bulkApproveTeachers() {
  ...
  emails.forEach(email => {
    ...
    const user = users.find(u => u.email.toLowerCase() === email && u.school === currentUser.school);
    if (user) { user.role = 'teacher'; user.isTeacher = true; user.approved = true; }
  });
  ls('ss_approvedTeachers_' + currentUser.school, teachers);
  ls('ss_users', users);
  renderApprovedTeachers();
...
function removeTeacher(email) {
  ls('ss_approvedTeachers_' + currentUser.school, lsArr('ss_approvedTeachers_' + currentUser.school).filter(e => e !== email));
  const users = lsArr('ss_users');
  const user = users.find(u => u.email.toLowerCase() === email && u.school === currentUser.school);
  if (user && !user.isAdmin) { user.role = 'student'; user.isTeacher = false; ls('ss_users', users); }
  renderApprovedTeachers();
}
