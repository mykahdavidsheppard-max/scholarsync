...
function launchApp(){
  ...
  renderCarpool();renderChats();renderStudyGroups();renderReminders();renderProfile();initNotificationBadges();renderActivityFeed();
  nav('today');
}
...
async function approveTeacher(){
  ...
  if(!email)return;
  const school=document.getElementById('teacher-school-input')?.value||currentUserData.school;
  ...
  if(!snap.empty)await db.collection('users').doc(snap.docs[0].id).update({isTeacher:true,role:'teacher'});
  renderApprovedTeachers();
  document.getElementById('teacher-email-input').value='';
  alert('Teacher access granted at '+school+'!');
}

async function grantAdmin(){
  const email=document.getElementById('admin-email-input').value.trim().toLowerCase();
  if(!email)return;
  const school=document.getElementById('admin-school-input')?.value||currentUserData.school;
  ...
  const snap=await db.collection('users').where('email','==',email).where('school','==',school).get();
  if(!snap.empty){
    await db.collection('users').doc(snap.docs[0].id).update({isAdmin:true,isTeacher:true,role:'teacher'});
  }else{
    // Pre-grant even if they haven't signed up yet — roles are stored in schoolRoles
    alert(`Pre-granted admin to ${email} at ${school}. They will get admin access when they sign up.`);
  }
  renderApprovedAdmins();
  document.getElementById('admin-email-input').value='';
  if(document.getElementById('admin-school-input'))document.getElementById('admin-school-input').value='';
  alert(`Admin access granted to ${email} at ${school}!`);
}
