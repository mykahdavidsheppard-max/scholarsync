  ...
  const unsub=db.collection('studentAbsences').where('uid','==',currentUser.uid).onSnapshot(snap=>{
    ...
    if(logEl)logEl.innerHTML=allItems.length?allItems.map(i=>{
      ...
      return`<div class="abs-log-item">
        ...
        ${i.fromTeacher?`<span style="font-size:10px;color:var(--blue-light);">📋 Teacher</span>`:''}
    ...
    .onSnapshot(snap=>{
      ...
      document.getElementById('ann-list').innerHTML=items.length?items.map(a=>`
        <div class="ann-card${a.isEmergency?' emergency-alert':''}">
          ${a.isEmergency?'<div style="font-size:11px;color:#ef4444;font-weight:700;margin-bottom:6px;">🚨 EMERGENCY ALERT</div>':''}
...
function renderClassroomDetail(){
  ...
  const assignmentHTML=a=>`
    <div class="assignment-row" style="flex-direction:column;align-items:stretch;margin-bottom:8px;">
      ...
      ${a.link?`<a href="${a.link}" target="_blank" style="font-size:12px;color:var(--blue-light);margin-top:6px;display:inline-flex;align-items:center;gap:4px;">🔗 View Resource</a>`:''}
    ...
    <div style="background:var(--card2);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;">
      <div style="display:flex;align-items:center;justify-content:space-between;">
        <div style="flex:1;">
          ...
          ${a.desc?`<div style="font-size:12px;color:var(--gray-light);margin-top:4px;">${a.desc}</div>`:''}
          ${a.link?`<a href="${a.link}" target="_blank" style="font-size:12px;color:var(--blue-light);margin-top:6px;display:block;">🔗 ${a.link}</a>`:''}
        </div>
...
function renderClubs(){
  const unsub=db.collection('clubs').where('school','==',currentUserData.school).onSnapshot(snap=>{
    ...
    el.innerHTML=myClubs.length?myClubs.map(c=>{
      ...
      return`<div class="club-card" onclick="openClubPortal('${c.id}')">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px;">
          <div class="club-role-badge">${c.president===currentUser.uid?'👑 President':'Member'}</div>
    ...
    }).join(''):'<div class="empty-state" style="padding:40px 20px;text-align:center;"><div style="font-size:48px;margin-bottom:12px;">🎭</div><div style="font-size:16px;font-weight:700;color:var(--white);margin-bottom:8px;">No Clubs Yet</div><div style="font-size:13px;color:var(--gray);">Ask your club president for a code and tap Join by Code to get in!</div></div>';
...
function renderClubPortal(){
  ...
  mainContent.innerHTML=`
    <div class="page-header">
      ...
      ${currentClub.president===currentUser.uid?`<div style="display:flex;gap:8px;"><button class="btn-ghost" onclick="openClubRoles()">👑 Roles</button><button class="btn-blue" onclick="openModal('club-announce-modal')">+ Announcement</button></div>`:''}
    </div>
    <div class="tab-row">
  ...
  overlay.innerHTML=`
    <div style="width:90%;max-width:600px;">
      <div style="display:flex;gap:10px;margin-bottom:16px;">
        ...
        <button class="btn-ghost" onclick="closeSearch()">✕</button>
      </div>
...
async function runSearch(q){
  ...
  annSnap.docs.forEach(d=>{
    ...
    if(data.title?.toLowerCase().includes(q.toLowerCase())||data.body?.toLowerCase().includes(q.toLowerCase())){
      results.push({type:'announcement',icon:'📢',title:data.title,sub:data.body?.slice(0,60),action:`nav('announcements')`});
    }
  });
  // Search directory
  ...
  dirSnap.docs.forEach(d=>{
    ...
    if(data.name?.toLowerCase().includes(q.toLowerCase())||data.email?.toLowerCase().includes(q.toLowerCase())){
      results.push({type:'user',icon:'👤',title:data.name,sub:data.email+' • '+data.role,action:`nav('directory')`});
    }
  });
  // Search chats
  ...
  chatSnap.docs.forEach(d=>{
    ...
    if(data.name?.toLowerCase().includes(q.toLowerCase())){
      results.push({type:'chat',icon:'💬',title:data.name,sub:'Group chat',action:`openChat('${d.id}');nav('chat');closeSearch()`});
    }
  });
    ...
    const unsub=db.collection('tips').where('school','==',currentUserData.school).onSnapshot(snap=>{
      ...
      el.innerHTML=tips.length?tips.map(t=>`
        <div style="background:var(--card2);border:1px solid var(--border);border-radius:10px;padding:14px;margin-bottom:10px;">
          ...
          ${t.resolved?'<div style="font-size:11px;color:#22c55e;margin-top:8px;">✓ Resolved</div>':`<button class="btn-ghost btn-sm" style="margin-top:8px;" onclick="resolveTip('${t.id}')">Mark Resolved</button>`}
...
async function sendEmergencyAlert(){
  ...
  await db.collection('announcements').add({
    title:'🚨 EMERGENCY ALERT',body:msg,
...
async function renderLeaderboard(){
  ...
  el.innerHTML=ranked.length?ranked.map((u,i)=>`
    <div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border);">
      <div style="font-size:${i<3?'24':'16'}px;font-weight:700;color:${i===0?'#f59e0b':i===1?'#94a3b8':i===2?'#cd7c3a':'var(--gray)'};width:32px;text-align:center;">${i===0?'🥇':i===1?'🥈':i===2?'🥉':i+1}</div>
  ...
  return`<div style="background:var(--card);border:1px solid var(--border);border-radius:14px;padding:20px;margin-bottom:16px;">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;">
      <div>
        ...
        <div style="font-size:12px;color:var(--gray);">${isClosed?'🔒 Closed — Results Revealed':'🗳️ Active — Voting Open'}</div>
      </div>
...
function positionHTML(election,pos,isClosed){
  ...
  return`<div style="margin-bottom:16px;padding:14px;background:var(--card2);border-radius:10px;">
    <div style="font-size:14px;font-weight:700;color:var(--blue-light);margin-bottom:10px;">🏆 ${pos.title}</div>
    ...
    ${myVote?`<div style="font-size:11px;color:#22c55e;margin-top:8px;">✅ You voted in this race</div>`:''}
  </div>`;
}
...
function renderElectionPositionsForm(){
  ...
  el.innerHTML=newElectionPositions.map(pos=>`
    <div style="background:var(--card2);border-radius:8px;padding:12px;margin-bottom:8px;">
      <div style="font-size:13px;font-weight:700;color:var(--blue-light);margin-bottom:8px;">🏆 ${pos.title} <button onclick="removeElectionPosition('${pos.id}')" style="background:none;border:none;color:var(--red);cursor:pointer;font-size:14px;">×</button></div>
      ${pos.candidates.map(c=>`<div style="font-size:12px;color:var(--gray);padding:4px 0;border-bottom:1px solid var(--border);">👤 ${c.name}${c.bio?' — '+c.bio:''}</div>`).join('')}
  ...
  document.getElementById('club-roles-list').innerHTML=users.map(u=>{
    ...
    return`<div style="display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--border);">
      ...
      ${u.uid!==currentUser.uid?`<select onchange="assignClubRole('${u.uid}',this.value)" style="background:var(--card2);border:1px solid var(--border);border-radius:6px;padding:4px 8px;color:var(--white);font-size:12px;">
        ...
        <option value="transfer">⚡ Make President</option>
      </select>`:'<span style="font-size:11px;color:var(--gold);">👑 You</span>'}
    </div>`;
  }).join('');
...
async function listenClubMembers(clubId){
  ...
  el.innerHTML=userSnaps.filter(s=>s.exists).map(s=>{
    ...
    return`<div class="dir-item">
      ...
      <span style="font-size:11px;background:rgba(59,130,246,.1);color:${roleColor};padding:2px 8px;border-radius:10px;font-weight:600;">${isPresident?'👑 ':''} ${role}</span>
    </div>`;
  }).join('');
}
