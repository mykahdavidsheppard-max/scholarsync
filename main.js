...
async function renderReportsPage(){
  ...
  ['report-hp-class','report-att-class','report-grade-class','report-interim-class'].forEach(id=>{
...
async function populateInterimStudents(){
  const classId=document.getElementById('report-interim-class').value;
  const studentEl=document.getElementById('report-interim-student');
  ...
  const userSnaps=await Promise.all((cls.students||[]).map(u=>db.collection('users').doc(u).get()));
  studentEl.innerHTML='<option value="">Select student...</option>'+
    userSnaps.filter(s=>s.exists).map(s=>`<option value="${s.id}">${s.data().name}</option>`).join('');
}
window.populateInterimStudents=populateInterimStudents;

async function exportInterimReport(){
  const classId=document.getElementById('report-interim-class').value;
  const studentUid=document.getElementById('report-interim-student').value;
  const teacherNotes=document.getElementById('report-interim-notes').value.trim();
  if(!classId||!studentUid)return alert('Select a class and student first.');

  // Fetch all needed data
  const [classDoc,userDoc,absenceSnap]=await Promise.all([
    db.collection('classrooms').doc(classId).get(),
    db.collection('users').doc(studentUid).get(),
    db.collection('studentAbsences').where('uid','==',studentUid).where('classId','==',classId).get()
  ]);

  if(!classDoc.exists||!userDoc.exists)return alert('Could not load data.');
  const cls=classDoc.data();
  ...
  assignments.forEach(a=>{
    const g=(grades[a.id]||{})[studentUid];
  ...
  // Missing assignments = has a due date, is past due, and has no grade
  const today=new Date();today.setHours(0,0,0,0);
  ...
  const absentCount=absences.filter(a=>a.type==='Unexcused'||a.type==='Parent Excused'||a.type==='Medical'||a.type==='College Visit').length;
  const tardyCount=absences.filter(a=>a.type==='Tardy').length;

  // Hall pass count
  const passSnap=await db.collection('hallpasses').where('studentUid','==',studentUid).where('classId','==',classId).get();
  const passCount=passSnap.size;

  // Build PDF
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF({unit:'pt',format:'letter'});
  const W=612,ml=48,mr=48,contentW=W-ml-mr;
  const accentRed=[163,45,45];
  const accentGreen=[59,109,17];
  const grayDark=[40,40,40];
  const grayMid=[100,100,100];
  const grayLight=[180,180,180];
  const bgRed=[252,235,235];
  ...
  doc.text(cls.school||currentUserData.school,ml,y+16);
  ...
  // Student info grid
  doc.setTextColor(...grayMid);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
  doc.text('STUDENT',ml,y);doc.text('GRADE',ml+contentW/4,y);
  doc.text('TEACHER',ml+contentW/2,y);doc.text('CLASS',ml+contentW*3/4,y);
  y+=13;
  doc.setTextColor(...grayDark);doc.setFontSize(11);doc.setFont('helvetica','bold');
  doc.text(student.name,ml,y);
  doc.setFont('helvetica','normal');
  doc.text(student.grade||'—',ml+contentW/4,y);
  doc.text(cls.teacherName||currentUserData.name,ml+contentW/2,y);
  const classNameShort=cls.name.length>22?cls.name.slice(0,22)+'…':cls.name;
  doc.text(classNameShort,ml+contentW*3/4,y);
  y+=22;

  // Divider
  doc.setDrawColor(...grayLight);doc.setLineWidth(0.5);
  doc.line(ml,y,W-mr,y);y+=16;

  // Stats row — average card + attendance card
  const cardH=70,cardW=(contentW-12)/2;

  // Average card
  doc.setFillColor(...bgGray);
  doc.roundedRect(ml,y,cardW,cardH,6,6,'F');
  doc.setTextColor(...grayMid);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
  doc.text('CURRENT AVERAGE',ml+cardW/2,y+14,{align:'center'});
  const avgColor=avg===null?grayMid:avg>=80?accentGreen:avg>=70?[24,95,165]:accentRed;
  doc.setTextColor(...avgColor);doc.setFontSize(30);doc.setFont('helvetica','bold');
  doc.text(avg!==null?avg+'%':'—',ml+cardW/2,y+46,{align:'center'});
  doc.setFontSize(10);doc.setFont('helvetica','normal');doc.setTextColor(...grayMid);
  doc.text(letterGrade,ml+cardW/2,y+62,{align:'center'});

  // Attendance card
  const ax=ml+cardW+12;
  doc.setFillColor(...bgGray);
  doc.roundedRect(ax,y,cardW,cardH,6,6,'F');
  doc.setTextColor(...grayMid);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
  doc.text('ATTENDANCE',ax+8,y+14);
  doc.setFontSize(10);doc.setTextColor(...grayDark);
  doc.text('Absences',ax+8,y+29);doc.setFont('helvetica','bold');doc.text(String(absentCount),ax+cardW-10,y+29,{align:'right'});
  doc.setFont('helvetica','normal');
  doc.text('Tardies',ax+8,y+43);doc.setFont('helvetica','bold');doc.text(String(tardyCount),ax+cardW-10,y+43,{align:'right'});
  doc.setFont('helvetica','normal');
  doc.text('Hall passes used',ax+8,y+57);doc.setFont('helvetica','bold');doc.text(String(passCount),ax+cardW-10,y+57,{align:'right'});

  y+=cardH+20;

  // Missing assignments section
  doc.setTextColor(...grayMid);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
  doc.text('MISSING ASSIGNMENTS',ml,y);
  if(missing.length>0){
    // Red badge
    doc.setFillColor(...bgRed);
    const badgeText=`${missing.length} missing`;
    doc.setFontSize(8.5);
    const badgeW=doc.getTextWidth(badgeText)+12;
    doc.roundedRect(ml+doc.getTextWidth('MISSING ASSIGNMENTS')+8,y-9,badgeW,13,4,4,'F');
    doc.setTextColor(...accentRed);
    doc.text(badgeText,ml+doc.getTextWidth('MISSING ASSIGNMENTS')+14,y);
  }
  y+=12;

  if(missing.length===0){
    doc.setFillColor(...bgGray);
    doc.roundedRect(ml,y,contentW,28,6,6,'F');
    doc.setTextColor(...grayMid);doc.setFontSize(10);doc.setFont('helvetica','italic');
    doc.text('No missing assignments — great work!',ml+contentW/2,y+17,{align:'center'});
    y+=44;
  }else{
    // Table header
    const rowH=22,col2=contentW*0.55,col3=contentW*0.78;
    doc.setFillColor(...bgRed);
    doc.rect(ml,y,contentW,rowH,'F');
    doc.setTextColor(...accentRed);doc.setFontSize(9);doc.setFont('helvetica','bold');
    doc.text('Assignment',ml+8,y+14);
    doc.text('Category',ml+col2,y+14);
    doc.text('Due Date',ml+col3,y+14);
    y+=rowH;
    missing.forEach((a,i)=>{
      if(i%2===0){doc.setFillColor(255,255,255);}else{doc.setFillColor(253,248,248);}
      doc.rect(ml,y,contentW,rowH,'F');
      doc.setDrawColor(...grayLight);doc.setLineWidth(0.4);
      doc.line(ml,y,ml+contentW,y);
      doc.setTextColor(...grayDark);doc.setFontSize(9.5);doc.setFont('helvetica','normal');
      const aName=a.name.length>38?a.name.slice(0,38)+'…':a.name;
      doc.text(aName,ml+8,y+14);
      doc.setTextColor(...grayMid);
      doc.text(a.category||'Assignment',ml+col2,y+14);
      doc.setTextColor(...accentRed);doc.setFont('helvetica','bold');
      const dueLabel=new Date(a.due).toLocaleDateString('en-US',{month:'short',day:'numeric'});
      doc.text(dueLabel,ml+col3,y+14);
      y+=rowH;
    });
    // Bottom border
    doc.setDrawColor(...grayLight);doc.line(ml,y,ml+contentW,y);
    y+=16;
  }
  ...
  if(teacherNotes){
    y+=4;
    doc.setTextColor(...grayMid);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
    doc.text('TEACHER NOTES',ml,y);y+=12;
    doc.setFillColor(...bgGray);
    const noteLines=doc.splitTextToSize(teacherNotes,contentW-20);
    const noteH=noteLines.length*14+16;
    doc.roundedRect(ml,y,contentW,noteH,6,6,'F');
    doc.setTextColor(...grayDark);doc.setFontSize(10);doc.setFont('helvetica','italic');
    doc.text(noteLines,ml+10,y+14);
    y+=noteH+16;
  }
  ...
  doc.setTextColor(...grayLight);doc.setFontSize(8.5);doc.setFont('helvetica','normal');
  ...
  doc.save(`InterimReport_${student.name.replace(/\s+/g,'_')}_${cls.name.replace(/\s+/g,'_')}.pdf`);
}
