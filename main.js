...
async function renderAbsences(){
  // Get manual absences from localStorage
  const localItems=lsArr('ss_absences_'+currentUser.uid);
  // Get teacher-logged absences from Firestore
  const snap=await db.collection('studentAbsences').where('uid','==',currentUser.uid).where('school','==',currentUserData.school).get();
  ...
  if(logEl)logEl.innerHTML=allItems.length?allItems.map(i=>{
    ...
    return`<div class="abs-log-item">
      ...
      ${i.fromTeacher?`<span style="font-size:10px;color:var(--blue-light);">📋 Teacher logged</span>`:''}
