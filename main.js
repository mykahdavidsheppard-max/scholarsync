...
function getMyMascot(){ return SCHOOL_MASCOTS[currentUserData?.school]||currentUserData?.school||'Home'; }
...
async function addCandidateToExistingElection(electionId,positionId){
  ...
  const bio=prompt('Campaign bio (optional):');
  ...
  if(!pos)return;
  pos.candidates=[...(pos.candidates||[]),{id:genId(),name,bio:bio||''}];
...
async function setStatus(statusKey){
  ...
  updateStatusDisplay();
  closeAllModals();
  showToast('Status updated to '+status.label,'success');
}
...
function statusDotHTML(user, size=10){
  if(!user.status||user.status==='online')return`<span style="display:inline-block;width:${size}px;height:${size}px;border-radius:50%;background:#22c55e;flex-shrink:0;"></span>`;
  const s=STATUS_OPTIONS.find(o=>o.key===user.status);
  return`<span style="display:inline-block;width:${size}px;height:${size}px;border-radius:50%;background:${s?.color||'#22c55e'};flex-shrink:0;" title="${s?.label||'Online'}"></span>`;
}

function renderStatusPicker(){
  const el=document.getElementById('status-picker');if(!el)return;
  el.innerHTML=STATUS_OPTIONS.map(s=>`
    <button onclick="setStatus('${s.key}')" style="display:flex;align-items:center;gap:10px;width:100%;padding:10px 14px;background:${currentUserData.status===s.key?'rgba(59,130,246,.1)':'transparent'};border:1px solid ${currentUserData.status===s.key?'#3b82f6':'var(--border)'};border-radius:10px;cursor:pointer;margin-bottom:6px;">
      <span style="width:12px;height:12px;border-radius:50%;background:${s.color};flex-shrink:0;"></span>
      <span style="color:var(--white);font-size:14px;">${s.label}</span>
      ${currentUserData.status===s.key?'<span style="margin-left:auto;color:#3b82f6;font-size:12px;">Current</span>':''}
    </button>`).join('');
}
...
function msgReactionsHTML(msg, collection, docId){
  const reactions=msg.reactions||{};
  const emojis=['👍','❤️','😂','🔥','👏'];
  const counts={};
  emojis.forEach(e=>{counts[e]=0;});
  Object.values(reactions).forEach(userR=>{Object.keys(userR).forEach(e=>{if(counts[e]!==undefined)counts[e]++;});});
  const myR=reactions[currentUser.uid]||{};
  const hasReactions=Object.values(counts).some(c=>c>0);
  return`<div style="display:flex;gap:4px;margin-top:4px;flex-wrap:wrap;">
    ${hasReactions?emojis.filter(e=>counts[e]>0).map(e=>`<span onclick="reactToMessage('${collection}','${docId}','${msg.id}','${e}')" style="background:${myR[e]?'rgba(59,130,246,.2)':'rgba(255,255,255,.08)'};border:1px solid ${myR[e]?'#3b82f6':'rgba(255,255,255,.1)'};border-radius:12px;padding:2px 8px;font-size:12px;cursor:pointer;">${e} ${counts[e]}</span>`).join(''):''}
    <span onclick="showReactionPicker('${collection}','${docId}','${msg.id}')" style="background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:2px 8px;font-size:12px;cursor:pointer;color:var(--gray);">+</span>
  </div>`;
}

function showReactionPicker(collection, docId, msgId){
  const existing=document.getElementById('reaction-picker-popup');
  if(existing)existing.remove();
  const picker=document.createElement('div');
  picker.id='reaction-picker-popup';
  picker.style.cssText='position:fixed;bottom:100px;left:50%;transform:translateX(-50%);background:#1e293b;border:1px solid var(--border);border-radius:16px;padding:10px 14px;display:flex;gap:8px;z-index:9999;box-shadow:0 10px 30px rgba(0,0,0,.5);';
  picker.innerHTML=['👍','❤️','😂','🔥','👏','😮','😢'].map(e=>`<span onclick="reactToMessage('${collection}','${docId}','${msgId}','${e}');this.parentElement.remove();" style="font-size:24px;cursor:pointer;transition:transform .1s;" onmouseover="this.style.transform='scale(1.3)'" onmouseout="this.style.transform='scale(1)'">${e}</span>`).join('');
...
function seenReceiptHTML(msg){
  ...
  if(!count||msg.uid!==currentUser.uid)return '';
  return`<div style="font-size:10px;color:var(--gray);text-align:right;margin-top:2px;">
    <svg viewBox="0 0 24 24" fill="none" stroke="#22c55e" stroke-width="2.5" width="12" height="12" style="vertical-align:middle;"><polyline points="20,6 9,17 4,12"/></svg>
    Seen by ${count}
  </div>`;
}
...
async function startCall(roomId, roomName, isTeacher){
  // Check if user is suspended from calls
  ...
  // Store active call in Firestore so others see it
  const callRef = db.collection('activeCalls').doc(roomId);
  await callRef.set({
    roomId, roomName,
  ...
  openJitsiCall(roomId, roomName, isTeacher);
}
...
async function joinCall(roomId, roomName){
  ...
  _activeCallId = roomId;
  openJitsiCall(roomId, roomName, false);
}

function openJitsiCall(roomId, roomName, isHost){
  // Remove existing call UI if any
  const existing = document.getElementById('call-overlay');
  if(existing) existing.remove();

  // Create full screen call overlay
  const overlay = document.createElement('div');
  ...
  overlay.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 20px;background:#1e293b;border-bottom:1px solid #334155;">
      <div>
        ...
        <div style="font-size:12px;color:#64748b;" id="call-participant-count">Loading...</div>
      </div>
      <div style="display:flex;gap:8px;">
    ...
    ${isHost?`<div style="background:#1e293b;border-top:1px solid #334155;padding:12px 20px;">
      <div style="font-size:12px;color:#64748b;margin-bottom:8px;font-weight:700;">PARTICIPANTS</div>
      <div id="call-participants-list" style="display:flex;gap:8px;flex-wrap:wrap;"></div>
    </div>`:''}
  `;
  ...
  if(!window.JitsiMeetExternalAPI){
    ...
    script.onload = () => initJitsi(roomId, roomName, isHost);
    document.head.appendChild(script);
  }else{
    initJitsi(roomId, roomName, isHost);
  }

  // Listen for participants in real time
  if(window._callUnsub) window._callUnsub();
  window._callUnsub = db.collection('activeCalls').doc(roomId).onSnapshot(snap=>{
...
function initJitsi(roomId, roomName, isHost){
  const container = document.getElementById('jitsi-container');
  if(!container) return;

  // Clean room name for Jitsi (no spaces or special chars)
  const cleanRoom = 'scholarsync_' + roomId.replace(/[^a-zA-Z0-9]/g,'_');
  ...
  _jitsiApi.addEventListener('participantLeft', ({id})=>{
    updateCallParticipants();
  });
  _jitsiApi.addEventListener('participantJoined', ()=>{
    updateCallParticipants();
  });
}

async function updateCallParticipants(){
  if(!_jitsiApi) return;
  const el = document.getElementById('call-participants-list');
  if(!el) return;
  try{
    const participants = _jitsiApi.getParticipantsInfo();
    el.innerHTML = participants.map(p=>`
      <div style="background:#1a56db22;border:1px solid #1a56db44;border-radius:8px;padding:6px 12px;display:flex;align-items:center;gap:8px;">
        <span style="font-size:13px;color:white;">${p.displayName||'Student'}</span>
        <button onclick="muteParticipant('${p.participantId}')" style="background:none;border:1px solid #64748b;border-radius:4px;color:#94a3b8;font-size:10px;padding:2px 6px;cursor:pointer;">Mute</button>
        <button onclick="kickParticipant('${p.participantId}')" style="background:none;border:1px solid #ef4444;border-radius:4px;color:#ef4444;font-size:10px;padding:2px 6px;cursor:pointer;">Kick</button>
      </div>
    `).join('');
  }catch(e){}
}
...
function muteParticipant(participantId){
  if(_jitsiApi) _jitsiApi.executeCommand('muteEveryone');
}

function kickParticipant(participantId){
  if(_jitsiApi) _jitsiApi.executeCommand('kickParticipant', participantId);
}

async function leaveCall(){
  if(_jitsiApi){ _jitsiApi.dispose(); _jitsiApi = null; }
  if(_activeCallId){
...
// Hook into announcements to send notifs
function renderAnnouncementsWithNotifs(){
  let firstLoad=true;
  const unsub=db.collection('announcements')
    .where('school','==',currentUserData.school)
    .onSnapshot(snap=>{
      ...
      if(annEl)annEl.innerHTML=items.length?items.map(a=>`
        <div class="ann-card${a.isEmergency?' emergency-alert':''}">
          ${a.isEmergency?`<div style="font-size:11px;color:#ef4444;font-weight:700;margin-bottom:6px;"><svg viewBox="0 0 24 24" fill="none" stroke="#ef4444" stroke-width="2" width="14" height="14" style="vertical-align:middle;margin-right:4px;"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> EMERGENCY ALERT</div>`:''}
          <div class="ann-title">${a.title}</div>
          <div class="ann-body">${a.body}</div>
          <div class="ann-meta"><span>${ICONS.pin} ${a.postedByName} • ${a.createdAt?.toDate?a.createdAt.toDate().toLocaleDateString():'Just now'}</span>${currentUserData.isAdmin?`<button class="btn-ghost btn-sm" onclick="deleteAnnouncement('${a.id}')">Delete</button>`:''}</div>
          ${reactionsHTML(a.id,a.reactions)}
      ...
      if(todayAnn)todayAnn.innerHTML=latest?`<div style="font-size:14px;font-weight:600;margin-bottom:4px;">${latest.isEmergency?'🚨 ':''} ${latest.title}</div><div style="font-size:12px;color:var(--gray)">${latest.body?.slice(0,100)}...</div>`:'<div class="empty-state">No announcements yet</div>';
    },err=>console.error('Announcements error:',err));
  unsubscribers.push(unsub);
}
...
async function setStatus(statusKey){
  await db.collection('users').doc(currentUser.uid).update({status:statusKey});
  currentUserData.status=statusKey;
  renderStatusPicker();
  closeAllModals();
}
...
// Override renderAnnouncements to use push notif version
window.renderAnnouncements=renderAnnouncementsWithNotifs;
