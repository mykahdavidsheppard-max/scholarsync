...
async function exportInterimReport(mode='download'){
  ...
  // Color helper — jsPDF needs explicit r,g,b — never pass array directly
  ...
  // Null-safety — jsPDF crashes on undefined/null
  const safe=(v,fallback='')=>v!=null?String(v):fallback;
  ...
  // ── STUDENT INFO ROW ──
  doc.setFontSize(8.5);doc.setFont('helvetica','normal');tc(COL.mid);
  doc.text('STUDENT',ml,y);doc.text('GRADE',ml+cW*0.35,y);doc.text('SCHOOL',ml+cW*0.55,y);
  y+=13;
  doc.setFontSize(13);doc.setFont('helvetica','bold');tc(COL.dark);
  ...
  y+=18;
  dc(COL.light);doc.setLineWidth(0.5);
  ...
  classData.forEach(({cls,avg,missing,absentCount,tardyCount})=>{
    ...
    // Class header bar
    fc(COL.classHeaderBg);
    doc.roundedRect(ml,y,cW,28,5,5,'F');
    doc.setFontSize(11);doc.setFont('helvetica','bold');tc(COL.classHeaderText);
    ...
    tc(avg!==null?avgColor(avg):COL.mid);
    doc.text(gradeStr,W-mr-8,y+18,{align:'right'});
    doc.setFontSize(8.5);doc.setFont('helvetica','normal');tc(COL.headerMuted);
    doc.text(safe(cls.teacherName),ml+10,y+26);
    y+=34;

    // Attendance mini row
    doc.setFontSize(9);doc.setFont('helvetica','normal');tc(COL.mid);
    ...
    if(missing.length===0){
      ...
      doc.setFontSize(9);doc.setFont('helvetica','italic');tc(COL.green);
      doc.text('No missing assignments',ml+cW/2,y+13,{align:'center'});
      y+=28;
    }else{
      fc(COL.bgRed);
      doc.rect(ml,y,cW,rowH,'F');
      doc.setFontSize(8.5);doc.setFont('helvetica','bold');tc(COL.red);
      ...
      missing.forEach((a,i)=>{
        ...
        if(i%2===0){doc.setFillColor(255,255,255);}else{fc(COL.missingRowAlt);}
        doc.rect(ml,y,cW,rowH,'F');
        dc(COL.redLight);doc.setLineWidth(0.3);
        doc.line(ml,y,ml+cW,y);
        doc.setFontSize(9);doc.setFont('helvetica','normal');tc(COL.dark);
        ...
        tc(COL.red);doc.setFont('helvetica','bold');
        let dueStr='—';
      ...
      dc(COL.redLight);doc.line(ml,y,ml+cW,y);
      y+=12;
    }
    y+=6;
  });
  ...
  doc.setFontSize(8);doc.setFont('helvetica','normal');tc(COL.light);
