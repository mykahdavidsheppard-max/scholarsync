...
async function exportInterimReport(mode='download'){
  ...
  const avgColor=(a)=>a===null?COL.mid:a>=80?COL.green:a>=70?COL.blue:COL.red;
  ...
  doc.text(grade||'—',ml+cW*0.35,y);
  const schoolShort=school.length>32?school.slice(0,32)+'…':school;
  doc.text(schoolShort,ml+cW*0.55,y);
  y+=18;
  ...
  doc.text(overallAvg!==null?overallAvg+'%':'—',ml+cW*0.13,y+38,{align:'center'});
  // Divider
  ...
  doc.text(String(classes.length),ml+cW*0.43,y+38,{align:'center'});
  // Divider
  ...
  doc.text(String(totalMissing),ml+cW*0.79,y+38,{align:'center'});
  y+=66;
  ...
  classData.forEach(({cls,avg,missing,absentCount,tardyCount})=>{
    ...
    const clsName=cls.name.length>40?cls.name.slice(0,40)+'…':cls.name;
    doc.text(clsName,ml+10,y+18);
    // Grade pill
    const gradeStr=avg!==null?avg+'%':'No grades yet';
    ...
    doc.text(cls.teacherName||'',ml+10,y+26);
    y+=34;

    // Attendance mini row
    doc.setFontSize(9);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
    doc.text(`Absences: ${absentCount}`,ml+8,y);
    doc.text(`Tardies: ${tardyCount}`,ml+90,y);
    y+=14;
    ...
    if(missing.length===0){
      ...
      missing.forEach((a,i)=>{
        ...
        const aName=a.name.length>44?a.name.slice(0,44)+'…':a.name;
        doc.text(aName,ml+8,y+13);
        doc.setTextColor(...COL.mid);
        doc.text(a.category||'Assignment',ml+cW*0.62,y+13);
        doc.setTextColor(...COL.red);doc.setFont('helvetica','bold');
        const dueStr=new Date(a.due).toLocaleDateString('en-US',{month:'short',day:'numeric'});
