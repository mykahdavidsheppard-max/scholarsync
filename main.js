  ...
  if(user){
    ...
    if(snap.exists&&snap.data().removed){
      ...
      if(parentSnap.exists){
        ...
        // No matching account at all (not student/teacher/admin, not parent, not a reset account).
        // This should not normally happen now that "Remove from School" resets rather than deletes.
        alert('Your account could not be found. Please sign up again.');
        await auth.signOut();
        document.getElementById('auth-screen').style.display='flex';
        document.getElementById('app').style.display='none';
      }
