...
async function signUp(){
  ...
  try{
    ...
    // ONLY founder OR explicitly granted admins get admin — first signup at school does NOT auto-admin
    ...
    // Only founder gets schoolRoles auto-created
    if(isFounder){await db.collection('schoolRoles').doc(school).set({admins:[email.toLowerCase()],teachers:[]},{merge:true});}
    // Manually trigger app launch since onAuthStateChanged may have fired before user doc was created
    ...
    const msgs={'auth/email-already-in-use':'Account already exists. Please sign in instead.','auth/weak-password':'Password too short.','auth/invalid-email':'Invalid email.'};
