...
async function signOut(){
  ...
  if(rejoinScreen)rejoinScreen.style.display='none';
...
auth.onAuthStateChanged(async user=>{
  if(user){
    ...
    if(snap.exists&&snap.data().removed){
      // Account was reset via "Remove from School" — same login, needs to re-enter school info
      ...
      if(parentSnap.exists){
        ...
        // Truly orphaned account (doc was hard-deleted before the reset-based fix existed).
        // Self-heal: rebuild a minimal profile flagged as removed, then route to Rejoin so
        // this account never dead-ends again — same login works going forward.
        await db.collection('users').doc(user.uid).set({
          uid:user.uid,
          name:user.displayName||user.email.split('@')[0],
          email:user.email.toLowerCase(),
          school:null,grade:null,
          role:'student',isAdmin:false,isTeacher:false,
          removed:true,
          removedAt:FS.serverTimestamp(),
          removedBy:'system (account recovery)',
          avatar:'linear-gradient(135deg,#1a56db,#3b82f6)',
          parentLinkCode:genParentLinkCode(),
          createdAt:FS.serverTimestamp()
        });
        currentUserData={school:null,grade:null,removed:true};
        document.getElementById('auth-screen').style.display='none';
        document.getElementById('app').style.display='none';
        showRejoinScreen();
      }
...
async function removeFromSchool(uid,name){
  if(!currentUserData.isAdmin)return;
  if(!confirm(`Remove ${name} from this school? Their account will be reset so they can sign back in and re-enter their info (same email/password). This is meant for cleaning up test or duplicate accounts.\n\nNote: this does NOT delete their login credentials — only Firebase Auth account deletion can do that, which requires backend access. This resets their school data instead.`))return;
  try{
    await db.collection('users').doc(uid).update({
      removed:true,
      removedAt:FS.serverTimestamp(),
      removedBy:currentUserData.name,
      school:null,
      grade:null,
      isAdmin:false,
      isTeacher:false,
    ...
    alert(`${name} has been removed from the school. They can sign back in with their existing email/password and will be prompted to re-enter their school info.`);
    filterDir();
...
function renderHelpGuide(){
  ...
  content.innerHTML=`
    ...
    <div class="card" style="margin-top:14px;border:1px solid var(--gold);">
      ...
      <p style="font-size:14px;color:var(--gray-light);line-height:1.6;">
        Use Remove from School in the Directory tab to reset duplicate or test accounts. This does NOT delete their login —
        the next time they sign in with their existing email/password, they'll be prompted to re-enter their school and grade.
      </p>
    </div>`:''}
  `;
}
...
// ============================================================
// REJOIN FLOW (for accounts reset via Remove from School)
// ============================================================

function showRejoinScreen(){
  document.getElementById('rejoin-screen').style.display='flex';
}

async function completeRejoin(){
  const school=document.getElementById('rejoin-school').value;
  const grade=document.getElementById('rejoin-grade').value;
  if(!school||!grade)return alert('Please select your school and grade.');
  try{
    // Check if this email was pre-granted teacher/admin at the new school
    const rolesSnap=await db.collection('schoolRoles').doc(school).get();
    const rolesData=rolesSnap.exists?rolesSnap.data():{};
    const isFounder=currentUserData.email?.toLowerCase()==='mykahsheppard@g.horrycountyschools.net';
    const isGrantedAdmin=(rolesData.admins||[]).includes(currentUserData.email?.toLowerCase());
    ...
    await db.collection('users').doc(currentUser.uid).update({
      ...
      removed:false,
      removedAt:null,
      parentLinkCode:genParentLinkCode() // issue a fresh code since the old one is no longer meaningful
    });

    document.getElementById('rejoin-screen').style.display='none';
    // Re-trigger the auth flow cleanly by reloading the fresh user doc
...
window.showRejoinScreen=showRejoinScreen;
window.completeRejoin=completeRejoin;


// ============================================================
// INTERIM PROGRESS REPORT — ALL CLASSES, SEARCH-BASED
// ============================================================

let interimSelectedStudent=null;
let interimSearchTimeout=null;

function searchInterimStudent(){
  clearTimeout(interimSearchTimeout);
  const q=document.getElementById('interim-search-input').value.trim().toLowerCase();
  const resultsEl=document.getElementById('interim-search-results');
  if(q.length<2){resultsEl.style.display='none';return;}
  interimSearchTimeout=setTimeout(async()=>{
    const snap=await db.collection('users').where('school','==',currentUserData.school).get();
    const matches=snap.docs.map(d=>d.data())
      .filter(u=>!u.isTeacher&&!u.isAdmin&&u.name.toLowerCase().includes(q))
      .slice(0,8);
    if(!matches.length){
      resultsEl.innerHTML='<div style="padding:12px;font-size:13px;color:var(--gray);">No students found</div>';
      resultsEl.style.display='';
      return;
    }
    resultsEl.innerHTML=matches.map(u=>`
      <div onclick="selectInterimStudent('${u.uid}','${u.name.replace(/'/g,"\\'")}','${(u.grade||'').replace(/'/g,"\\'")}','${(u.school||'').replace(/'/g,"\\'")}')"
        style="padding:10px 14px;cursor:pointer;border-bottom:1px solid var(--border);font-size:13px;color:var(--white);"
        onmouseover="this.style.background='var(--card)'" onmouseout="this.style.background=''">
        <div style="font-weight:700;">${u.name}</div>
        <div style="font-size:11px;color:var(--gray);">${u.grade||'—'} • ${u.school}</div>
      </div>`).join('');
    resultsEl.style.display='';
  },300);
}

function selectInterimStudent(uid,name,grade,school){
  interimSelectedStudent={uid,name,grade,school};
  document.getElementById('interim-search-input').value='';
  document.getElementById('interim-search-results').style.display='none';
  const selBox=document.getElementById('interim-selected-student');
  selBox.style.display='flex';
  document.getElementById('interim-selected-name').textContent=name;
  document.getElementById('interim-selected-meta').textContent=`${grade} • ${school}`;
  document.getElementById('interim-action-btns').style.display='flex';
  document.getElementById('interim-status').textContent='';
}

function clearInterimStudent(){
  interimSelectedStudent=null;
  document.getElementById('interim-selected-student').style.display='none';
  document.getElementById('interim-action-btns').style.display='none';
  document.getElementById('interim-status').textContent='';
}

// Close dropdown when clicking outside
document.addEventListener('click',e=>{
  const wrap=document.getElementById('interim-search-input')?.closest('.card');
  if(wrap&&!wrap.contains(e.target)){
    const r=document.getElementById('interim-search-results');
    if(r)r.style.display='none';
  }
});

async function exportInterimReport(mode='download'){
  if(!interimSelectedStudent)return alert('Search for and select a student first.');
  const statusEl=document.getElementById('interim-status');
  statusEl.textContent='Building report…';

  const {uid,name,grade,school}=interimSelectedStudent;

  // Fetch all classes the student is enrolled in
  const classSnap=await db.collection('classrooms').where('school','==',school).where('students','array-contains',uid).get();
  const classes=classSnap.docs.map(d=>({id:d.id,...d.data()}));

  if(!classes.length){statusEl.textContent='This student is not enrolled in any classes yet.';return;}

  // Fetch attendance across all classes
  const absenceSnap=await db.collection('studentAbsences').where('uid','==',uid).get();
  const absences=absenceSnap.docs.map(d=>d.data());
  ...
  const classData=classes.map(cls=>{
    ...
    assignments.forEach(a=>{
      const g=(grades[a.id]||{})[uid];
      if(g!==undefined&&g!==null){
        totalScore+=Number(g)*(Number(a.weight)||1);
        totalWeight+=Number(a.weight)||1;
      }
    });
    const avg=totalWeight?Math.round(totalScore/totalWeight):null;
    ...
    const classAbsences=absences.filter(a=>a.classId===cls.id);
    const absentCount=classAbsences.filter(a=>['Unexcused','Parent Excused','Medical','College Visit'].includes(a.type)).length;
    const tardyCount=classAbsences.filter(a=>a.type==='Tardy').length;
    return {cls,avg,missing,absentCount,tardyCount};
  }).sort((a,b)=>{
  ...
  // Overall average across all classes that have grades
  const graded=classData.filter(d=>d.avg!==null);
  const overallAvg=graded.length?Math.round(graded.reduce((s,d)=>s+d.avg,0)/graded.length):null;
  const totalMissing=classData.reduce((s,d)=>s+d.missing.length,0);

  // Build PDF
  const {jsPDF}=window.jspdf;
  const doc=new jsPDF({unit:'pt',format:'letter'});
  const W=612,ml=48,mr=48,cW=W-ml-mr;

  const COL={
    headerBg:[22,31,48], headerText:[240,244,255], headerMuted:[180,190,210],
    dark:[30,30,30], mid:[100,100,100], light:[190,190,190],
    bgGray:[246,246,246], bgRed:[252,235,235], red:[163,45,45], redLight:[240,149,149],
    green:[59,109,17], blue:[24,95,165],
    classHeaderBg:[30,41,60], classHeaderText:[200,215,240],
    missingRowAlt:[253,248,248],
  };

  // Null-safety helper — jsPDF crashes hard on undefined/null/non-string
  ...
  doc.text('ScholarSync',ml,y);
  doc.setFontSize(10);doc.setFont('helvetica','normal');
  doc.setTextColor(...COL.headerMuted);
  doc.text(safe(school),ml,y+16);
  doc.setFontSize(12);doc.setFont('helvetica','bold');
  doc.setTextColor(...COL.headerText);
  doc.text('Interim Progress Report',W-mr,y,{align:'right'});
  doc.setFontSize(10);doc.setFont('helvetica','normal');
  doc.setTextColor(...COL.headerMuted);
  doc.text(`Generated ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}`,W-mr,y+16,{align:'right'});
  y=90;

  // ── STUDENT INFO ROW ──
  doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
  doc.text('STUDENT',ml,y);doc.text('GRADE',ml+cW*0.35,y);doc.text('SCHOOL',ml+cW*0.55,y);
  y+=13;
  doc.setFontSize(13);doc.setFont('helvetica','bold');doc.setTextColor(...COL.dark);
  doc.text(safe(name,'Unknown'),ml,y);
  doc.setFontSize(11);doc.setFont('helvetica','normal');
  doc.text(safe(grade,'—'),ml+cW*0.35,y);
  const schoolShort=safe(school);
  doc.text(schoolShort.length>32?schoolShort.slice(0,32)+'…':schoolShort,ml+cW*0.55,y);
  y+=18;
  doc.setDrawColor(...COL.light);doc.setLineWidth(0.5);
  doc.line(ml,y,W-mr,y);y+=14;

  // ── SUMMARY BANNER ──
  doc.setFillColor(...COL.bgGray);
  doc.roundedRect(ml,y,cW,52,6,6,'F');
  doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
  doc.text('OVERALL AVERAGE',ml+cW*0.13,y+12,{align:'center'});
  doc.setFontSize(22);doc.setFont('helvetica','bold');
  doc.setTextColor(...avgColor(overallAvg));
  doc.text(overallAvg!==null?safe(overallAvg+'%'):'—',ml+cW*0.13,y+38,{align:'center'});
  doc.setDrawColor(...COL.light);doc.line(ml+cW*0.28,y+8,ml+cW*0.28,y+44);
  doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
  doc.text('CLASSES',ml+cW*0.43,y+12,{align:'center'});
  doc.setFontSize(22);doc.setFont('helvetica','bold');doc.setTextColor(...COL.dark);
  doc.text(safe(classes.length,'0'),ml+cW*0.43,y+38,{align:'center'});
  doc.setDrawColor(...COL.light);doc.line(ml+cW*0.58,y+8,ml+cW*0.58,y+44);
  doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
  doc.text('TOTAL MISSING',ml+cW*0.79,y+12,{align:'center'});
  doc.setFontSize(22);doc.setFont('helvetica','bold');
  doc.setTextColor(totalMissing>0?COL.red:COL.green);
  doc.text(safe(totalMissing,'0'),ml+cW*0.79,y+38,{align:'center'});
  y+=66;

  // ── PER-CLASS SECTIONS ──
  const rowH=20;

  classData.forEach(({cls,avg,missing,absentCount,tardyCount})=>{
    const estimatedHeight=46+(missing.length>0?24+missing.length*rowH+8:28);
    if(y+estimatedHeight>720){doc.addPage();y=40;}

    // Class header bar
    doc.setFillColor(...COL.classHeaderBg);
    doc.roundedRect(ml,y,cW,28,5,5,'F');
    doc.setFontSize(11);doc.setFont('helvetica','bold');doc.setTextColor(...COL.classHeaderText);
    const rawClsName=safe(cls.name,'Untitled Class');
    const clsName=rawClsName.length>40?rawClsName.slice(0,40)+'…':rawClsName;
    doc.text(clsName,ml+10,y+18);
    const gradeStr=avg!==null?safe(avg+'%'):'No grades yet';
    doc.setFontSize(11);doc.setFont('helvetica','bold');
    doc.setTextColor(...(avg!==null?avgColor(avg):COL.mid));
    doc.text(gradeStr,W-mr-8,y+18,{align:'right'});
    doc.setFontSize(8.5);doc.setFont('helvetica','normal');doc.setTextColor(...COL.headerMuted);
    doc.text(safe(cls.teacherName),ml+10,y+26);
    y+=34;

    // Attendance mini row
    doc.setFontSize(9);doc.setFont('helvetica','normal');doc.setTextColor(...COL.mid);
    doc.text(`Absences: ${safe(absentCount,'0')}`,ml+8,y);
    doc.text(`Tardies: ${safe(tardyCount,'0')}`,ml+90,y);
    y+=14;
    ...
    if(missing.length===0){
      ...
      doc.setFontSize(9);doc.setFont('helvetica','italic');doc.setTextColor(...COL.green);
      doc.text('No missing assignments',ml+cW/2,y+13,{align:'center'});
      y+=28;
    }else{
      doc.setFillColor(...COL.bgRed);
      doc.rect(ml,y,cW,rowH,'F');
      doc.setFontSize(8.5);doc.setFont('helvetica','bold');doc.setTextColor(...COL.red);
      doc.text(`${missing.length} missing assignment${missing.length>1?'s':''}`,ml+8,y+13);
      doc.text('Category',ml+cW*0.62,y+13);
      doc.text('Due',W-mr-8,y+13,{align:'right'});
      y+=rowH;
      missing.forEach((a,i)=>{
        if(y>720){doc.addPage();y=40;}
        doc.setFillColor(...(i%2===0?[255,255,255]:COL.missingRowAlt));
        doc.rect(ml,y,cW,rowH,'F');
        doc.setDrawColor(...COL.redLight);doc.setLineWidth(0.3);
        doc.line(ml,y,ml+cW,y);
        doc.setFontSize(9);doc.setFont('helvetica','normal');doc.setTextColor(...COL.dark);
        const rawAName=safe(a.name,'Untitled');
        const aName=rawAName.length>44?rawAName.slice(0,44)+'…':rawAName;
        doc.text(aName,ml+8,y+13);
        doc.setTextColor(...COL.mid);
        doc.text(safe(a.category,'Assignment'),ml+cW*0.62,y+13);
        doc.setTextColor(...COL.red);doc.setFont('helvetica','bold');
        let dueStr='—';
        try{if(a.due)dueStr=new Date(a.due).toLocaleDateString('en-US',{month:'short',day:'numeric'});}catch(e){}
        doc.text(dueStr,W-mr-8,y+13,{align:'right'});
        y+=rowH;
      });
      doc.setDrawColor(...COL.redLight);doc.line(ml,y,ml+cW,y);
      y+=12;
    }
    y+=6;
  });
  ...
  doc.setFontSize(8);doc.setFont('helvetica','normal');doc.setTextColor(...COL.light);
  doc.text('ScholarSync — Horry County Schools',ml,footerY+12);
  doc.text('Confidential — for parent/guardian use only',W-mr,footerY+12,{align:'right'});

  const fileName=`InterimReport_${name.replace(/\s+/g,'_')}.pdf`;

  if(mode==='download'){
    doc.save(fileName);
    statusEl.textContent='✓ PDF downloaded.';
  }else if(mode==='parent'){
    // Save report to Firestore so parent can see it in their portal
    const pdfBase64=doc.output('datauristring');
    await db.collection('interimReports').add({
      studentUid:uid,studentName:name,school,
      pdfData:pdfBase64,
      generatedBy:currentUserData.name,
      generatedAt:FS.serverTimestamp()
    });
    doc.save(fileName);
    statusEl.textContent='✓ PDF downloaded and sent to Parent Portal.';
  }else if(mode==='link'){
    // Save to Firestore and copy a link the parent can open
    const ref=await db.collection('interimReports').add({
      studentUid:uid,studentName:name,school,
      pdfData:doc.output('datauristring'),
      generatedBy:currentUserData.name,
      generatedAt:FS.serverTimestamp()
    });
    const link=`${window.location.origin}${window.location.pathname}?interim=${ref.id}`;
    await navigator.clipboard.writeText(link).catch(()=>{});
    doc.save(fileName);
    statusEl.textContent='✓ PDF downloaded and link copied to clipboard.';
  }

  setTimeout(()=>{statusEl.textContent='';},4000);
}

window.searchInterimStudent=searchInterimStudent;
window.selectInterimStudent=selectInterimStudent;
window.clearInterimStudent=clearInterimStudent;
window.exportInterimReport=exportInterimReport;
